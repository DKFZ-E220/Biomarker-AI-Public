---
title: "GSEA loop — MSigDB C1-C8 and H, per input DEG list"
output: html_document
---

```{r setup, include=FALSE}
# Load paths and constants. Input DEG lists are expected under
# PRECLIN_TX_OUT (produced by 01_deseq_hpv.Rmd).
source(here::here("config.R"))
knitr::opts_chunk$set(echo = TRUE)
```

#trying one code for getting the merged result (Final Code)
...
input_files <- c(file.path(PRECLIN_TX_OUT, "deg_treated_lfchcange1.5.csv"), file.path(PRECLIN_TX_OUT, "deg_Untreated_fchcange1.5.csv"))

# Load OrgDb for humans
human_orgdb <- org.Hs.eg.db

# Specify the categories you want to analyze
categories_to_analyze <- c("C1", "C2", "C3", "C4", "C5", "C6", "C7", "C8", "H") 
...
results_directory <- file.path(PRECLIN_TX_OUT, "GSEAresults_loopcode")
...
save_merged_results <- function(all_results, input_file) {
  ...
  input_file_directory <- file.path(PRECLIN_TX_OUT, "GSEAresults_loopcode", tools::file_path_sans_ext(basename(input_file)))
...
#create dotplots and enrichplots
```{r}
# Load necessary libraries
library(ggplot2)

# Create base directories
#dir.create(file.path(results_directory, "enrichplots"), showWarnings = FALSE, recursive = TRUE)
...
for (category_name in names(all_results_list)) {
  ...
  for (input_file in input_files) {
    ...
    if (!is.null(result) && nrow(result) > 0) {  # Check if result is not null and has rows
      # Enrichplot
      #geneSetIDs <- result$ID
      #enrich_plot_list <- list()
      #for (geneSetID in geneSetIDs) {
        #enrich_plot <- gseaplot(
          #result,
          #geneSetID = geneSetID,
          #title = geneSetID,
          #color.line = "#0d76ff"
        #)
        #enrich_plot_list[[geneSetID]] <- enrich_plot
      #}
      input_file_name <- tools::file_path_sans_ext(basename(input_file))
      #save_plots(enrich_plot_list, file.path(results_directory, "enrichplots"), category_name, input_file_name)
      
            # Doseplot
      dose_plot <- dotplot(result, showCategory=30, split=".sign", font.size=8)
...
##if you want specifiic sub categories, heres how you do it
...
# List of input files (replace with your actual file names)
input_files <- c(file.path(PRECLIN_TX_OUT, "significant_degs_treated_lfc1.5.csv"), file.path(PRECLIN_TX_OUT, "significant_degs_untreated_lfc1.5.csv"))

# Load OrgDb for humans
human_orgdb <- org.Hs.eg.db

# Specify the categories and subcategories you want to analyze
categories_to_analyze <- list(
  C2 = c("CP:BIOCARTA", "CP:KEGG", "CP:PID", "CP:REACTOME", "CP:WIKIPATHWAYS"),
  C3 = c("TFT:GTRD", "TFT:TFT_LEGACY"),
  C5 = c("GO:BP", "GO:CC", "GO:MF")
)
...
# Create a list to store GSEA results for each subcategory and each input file
all_results_list <- list()

# Perform GSEA analysis for each category and each input file
for (main_category in names(categories_to_analyze)) {
  gene_sets <- msigdbr(species = "Homo sapiens", category = main_category)
  
  subcategories <- categories_to_analyze[[main_category]]
  
  for (subcategory in subcategories) {
    filtered_gene_sets <- gene_sets %>%
      filter(gs_subcat == subcategory)
    
    if (nrow(filtered_gene_sets) > 0) {
      subcategory_results <- list()
      
      for (input_file in input_files) {
        data <- read.csv(input_file)
        gsea_result <- perform_gsea(filtered_gene_sets, data)
        subcategory_results[[input_file]] <- gsea_result
      }
      
      all_results_list[[subcategory]] <- subcategory_results
    } else {
      print(paste("Subcategory not found:", subcategory))
    }
  }
}
...
generate_plots <- function(data, input_file_directory) {
  ...
  # Check if data is NULL, doesn't exist, or empty before proceeding
  if (!("gs_filtered" %in% names(data)) || !("gs_clustered" %in% names(data)) || 
      is.null(data$gs_filtered) || is.null(data$gs_clustered) || 
      length(data$gs_filtered) == 0 || length(data$gs_clustered) == 0) {
    warning(paste("No data to plot for directory:", input_file_directory))
    return()
  }
  # Generate dendrogram plot
  png(filename = file.path(input_file_directory, "dendrogram_plot.png"))
...
generate_plots <- function(data, input_file_directory) {
  ...
  if (is.null(data$gs_filtered) || is.null(data$gs_clustered) ||
      nrow(data$gs_filtered) == 0 || nrow(data$gs_clustered) == 0) {
    warning(paste("No data to plot for directory:", input_file_directory))
    return()
  }
